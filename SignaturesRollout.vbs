...
' PURPOSE: To programatically set the following Outlook Settings for employees; including the following 'features':
...
' LICENSE: This code is subject to the GNU General Public License v3.0. Which is translated to include:
'   Permissions of this strong copyright license are conditioned on making available complete source code 
'   of licensed works and modifications, which include larger works using a licensed work, under the same license.
'   Copyright and license notices must be preserved. Contributors provide an express grant of patent rights.
'
' BRIEF:
'   This email signature generation script connects directly from their PC's LOGONSERVER; retrie 
'    the users AD record, with a filter of included attributes. Then extracts that data to an XML document in memory.
'   The XML document is merged against XSL templates using the standard XML Transform method.
'   The output of this transform is a HTML/RTF/TXT compliant email signature; capable of being easily selected 
'    from Outlook's Compose email view (Signatures dropdown menu).
...
'   The script will only run on a windows machine that MUST be connected to the corporate network somehow (LAN/VPN etc).
'	Without additional arguments passed to the script; a simple double click on the VBS will use the currently logged on users 
'    session to communicate with AD. With a locked out user account, the signature generation will fail.
...
'		...perform conditional logic (XSL:IF, XSL:CHOOSE/WHEN and even regex, and even perform a simple lowercase)
' 
' PREPARATION INSTRUCTIONS
' This script (SignaturesRollout.vbs) needs to be stored on a network-accessible-share (\\NTDomainName\NETLOGON\Signatures maybe?)
' The extracted folder structure (explained below) should also exist.
' Keep the Basic.html.xlt, Basic.rtf.xlt and Basic.txt.xlt filenames in the /Templates folder; and write your own for your company
' Some guidence:
'	https://github.com/EDMdesigner/modern-html-email-tutorial
...
'	/Stationery				Deploy a marketing-approved pre-formatted email. Useful for sales/customer services departments
'	Company themed email letters (Stationery)
'
' USAGE
'	The following WINDOWS-ONLY command line syntax is:
'   wscript <signaturerollout.vbs> [/customdomain:test.fqdn /logging:true /path:<path to installation folder> /username:test.account /development:true]
'	wscript.exe %userprofile%\Documents\a-technicans-journey\Signatures\SignaturesRollout.vbs /username:sjackson0109 /logging:true
'
' HISTORY
...
' 	Implement a telephoneNumber attribute data conversion routine to E.164 number format
'	 (remove braces/brackets, replace leading digit with country code matching 'c' field' standard)
'	Implement a Conversion routine to E.123 number format (like E.164, only formatted with space separation to reading/typing easier)
'	New Method to perform a HTTP POST to the mail-server (microsoft exchange assumed), to commit their Outlook signature to their OWA Signature
'	New Method to leverage the 'store signatures inside mailbox information store', without using MAPI/CDO libraries
'
'
' Known Issues
' Outlook uses Microsoft Word as a default email editor; when attaching images (PNG, JPG or BMP formats as a minimum), the image is re-rastered to 96DPI. If you 
'  had a 56DPI image, this effectively shrinks your image height and width to fit the required number of pixels into the 'inch' specified.  Higher res images 
'  are blown-up/expand and may look rediculous. Try 200DPI with a picture around 1" tall, send the email and the recepient receives an image 2x the height and width!
'  Turning this feature off, is not desirable either; this causes other problems with Microsoft Word on it's own, outside of Outlook. 
'############################################################################
